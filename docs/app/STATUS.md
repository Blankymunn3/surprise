...
| 좌표 → 지역 (기기 안에서) | ✅ | ✅ |
| EXIF 날짜·위치 읽기 | ✅ | 아직 |
| 사진 줄이기 (760px / 품질 72) | ✅ | 아직 |
| Firebase Storage REST | ✅ | ✅ |
| 실제 지도(MapLibre) | ✅ 기본 지도 + 표시 | 아직 (앱 껍데기에 둘 예정) |
...
1. **지도에 지역을 사진으로 칠하기** — 지금은 표시만 찍습니다.
   MapLibre 에 런타임 이미지를 `fill-pattern` 으로 등록해야 합니다.
2. **iOS EXIF · 사진 줄이기** — ImageIO 로 안드로이드와 같은 값(760px, 0.72)을 씁니다.
3. **iOS 앱 껍데기(Xcode 프로젝트)** — 패키지는 다 만들어져 있고, Xcode 에서
   App 타깃을 하나 만들어 로컬 패키지를 걸면 됩니다. Pretendard 등록도 여기서.
4. **로그인(구글) + Firestore** — 위 두 임시 방식이 사라집니다.
5. **해외 시도(ADM1) 경계** — 웹은 '시도 나누기' 를 누를 때 받아옵니다. 앱도 같은 방식으로.
6. **웹을 새 구조에 맞추기** — 웹은 아직 `regions/<코드>.jpg` 한 장만 봅니다.
