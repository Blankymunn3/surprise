...
| 디자인 토큰 (design.html 그대로) | ✅ Pretendard 동봉 | ✅ Pretendard 등록됨 |
| 공간 목록 화면 | ✅ | ✅ |
| 지도 \| 달력 탭 | ✅ | ✅ |
| 달력 화면 | ✅ | ✅ |
| 사진 올리기 시트 | ✅ | ✅ |
...
1. **동화풍 그림** — `design.html` 의 언덕·나무·길처럼 면으로 채운 그림은 아직
   없습니다. 색·모서리·지도 결까지만 옮겼습니다. 그림은 두 플랫폼에 벡터로 새로
   그려야 해서 따로 합니다.
2. **로그인(구글) + Firestore** — 위 두 임시 방식이 사라집니다.
3. **해외 시도(ADM1) 경계** — 웹은 '시도 나누기' 를 누를 때 받아옵니다. 앱도 같은 방식으로.
4. **웹을 새 구조에 맞추기** — 웹은 아직 `regions/<코드>.jpg` 한 장만 봅니다.
