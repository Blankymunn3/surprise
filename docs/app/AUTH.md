...
> ✅ **1~4 는 구현했습니다** (2026-07-31, 커밋 `d0710ac`). 양쪽 빌드·테스트 통과.
> **5(화면)만 남았습니다** — 새 UI 라 디자인부터 하는 순서라서요. 그때까지 호출부는
> `SpaceKind.Shared` 고정이라 지금까지와 똑같이 동작합니다.
...
> ⚠️ 여기는 **아직 안 만들었습니다.** 새 UI 는 디자인 → 검수 → 개발 순서라서요.
> 아래는 방향이고, 문구와 모양은 디자인에서 정합니다.
