...
파일 이름이 `<사진ID>.jpg` 하나로 짧아졌습니다. 지역·날짜를 이름에 적던 것은 사진 정보를
둘 곳이 없어서였는데, 이제 문서가 있습니다.

**올린 시각도 진짜 값이 됐습니다.** 전에는 목록만 보고 흉내 낸 값(`stableOrder`)을 썼는데,
이제 문서에 적힙니다. 그 흉내는 **혼자 짜국에만** 남았습니다.

> **혼자 쓰는 짜국은 파일 이름 방식 그대로입니다** (`LocalPhotoRepository`).
> 기기 안에는 Firestore 가 없으니까요. `PhotoObjectName` 은 그래서 안 지웠습니다.

대표사진 규칙만 사진과 다릅니다 — **멤버 누구나 고칩니다.** 지도에 무엇을 걸어 둘지는
짜국의 공동 결정이라, 내가 올린 사진만 대표로 삼게 하면 오히려 이상합니다.
