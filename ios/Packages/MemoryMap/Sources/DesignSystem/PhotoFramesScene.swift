    ...
    public var body: some View {
        Canvas { context, size in
            ...
            func frame(_ left: CGFloat, _ top: CGFloat, fill: Color?) {
                ...
                context.stroke(Path(rect), with: .color(MemoryColor.ink), style: line)
            }

            // 뒤에서 앞으로. 앞의 것이 뒤의 것을 가려야 겹쳐 보입니다.
            frame(6, 18, fill: nil)
            frame(26, 34, fill: MemoryColor.fill)
            frame(46, 50, fill: MemoryColor.surface)

            context.fill(
                Path(CGRect(x: p(98), y: p(10), width: p(22), height: p(22))),
                with: .color(MemoryColor.accent)
            )
            ...
            context.stroke(ridge, with: .color(MemoryColor.ink), style: line)
        }
    }
}
